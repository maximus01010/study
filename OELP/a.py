...
def final_ans(url,index):
        ...
        try:
            movie=soup.find('span',class_='sc-7f1a92f5-1 benbRT').text
            print(movie)
...
with open(file_name1, 'w', newline='') as csvfile:
    ...
    # Write the header (column names)
    csv_writer.writerow(columns1)

    # Write the data rows
    csv_writer.writerows(garbage)